...
@Component({
  selector: 'app-find-lessons',
  templateUrl: './find-lessons.component.html',
  styleUrls: ['./find-lessons.component.scss']
})
export class FindLessonsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.userSubscription = this.userService.getCurrentUserObservable().subscribe(user => {
      ...
      if (this.currentUser) {
        ...
        this.calendarWeeks = [];
        this.snackBar.open('Please log in to book lessons.', 'Close', { duration: 3000 });
      }
    });
  }
